...
class CKFinder_Connector_Core_Config
{
     ...
     *
     * @var	array	$_xsendfileNginx	Configuration for comment => root
     */
	...
	*/
    ...
    /**
     * Get the dditional Nginx X-Sendfile configuration (comment => root)
     */
    public function getXSendfileNginx(){
      ...
      foreach ( $this->_xsendfileNginx as $location => $root ){
        ...
        if ( substr($root,-1,1) != '/' && substr($root,-1,1) != '\\') {
          // root and comment paths are concatenated
