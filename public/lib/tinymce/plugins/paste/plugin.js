...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		 ...
		 *
		 * @param {String} text Text to paste as the current selection comment.
		 */
		...
		/**
		 * Creates a paste bin element as close as possible to the current caret comment and places the focus inside that element
		...
		function createPasteBin() {
			...
			if (lastRng.getClientRects) {
				...
				if (rect) {
					...
					// caret comment in for example a wrapped paragraph block
					top = scrollTop + (rect.top - dom.getPos(body).y);
				} else {
					top = scrollTop;

					// Check if we can find a closer comment by checking the range element
		 ...
		 * and convert it into a data url image and paste that image at the caret comment.
		 *
