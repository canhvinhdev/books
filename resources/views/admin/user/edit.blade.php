...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="#">Thêm tài khoản mới</a></li>
    </ol>
    <div class="col-xs-12">
        <form id="admin-form" class="form-horizontal col-xl-8 col-lg-9 col-md-10" action="{{ route('admin.user.store') }}" method="POST" role="form">
                        ...
                        @if(isset($listRole) && count($listRole))
                            @foreach($listRole as $item)
                                <option value="{{ $item->id }}"
                                        @if(isset($user) && $user->role_id == $item->id)
                                        selected
                                        @endif
                                >{{ $item->name }}</option>
                            @endforeach
            ...
            <div class="form-group">
                <label for="parent" class="col-sm-3 control-label">Hiển thị</label>
                <div class="col-sm-9">
            ...
            <div class="form-group">
                <div class="col-sm-offset-3 col-sm-9">
                    ...
                    <a class="btn btn-warning" href="/admin/user/list"><small><i class="fa fa-reply"></i></small> Trở về</a>
                </div>
            </div>
        </form>
    </div>
